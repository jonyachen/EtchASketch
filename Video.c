...
void plot(int x, int y, int b) {
    ...
    if(x >= 0 && x < HRes && y >= 0 && y < VRes) {
        ...
        /* Might need to do something similar to this later if we want to  change resolution/enlarge image
        else {
        ...
        */
        ...
        // To-do - account for different colors, rather than just one
        // For now, just connect one color
      	if(b == 0)
            VideoBuf[y * (HBuf/32) + x/32] &= ~(0x80000000>>(x & 0x1f));   		    // turn off the pixel
        else if(b == -1)
        	VideoBuf[y * (HBuf/32) + x/32] ^= (0x80000000>>(x& 0x1f));			    // invert the pixel
